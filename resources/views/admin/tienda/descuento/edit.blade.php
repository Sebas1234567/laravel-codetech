...
@section('title')Edición de Descuentos | CodeTechEvolution @stop
...
<div class="card border-top-secondary border-top-3 mb-4">
  <div class="card-header d-flex align-items-center">
    ...
    <strong class="ms-2">Editar Descuento</strong>
  </div>
  <div class="card-body">
    <div class="tab-content rounded-bottom">
      ...
      <div class="tab-pane p-3 active preview" role="tabpanel" id="preview-1000">
        {{ html()->modelForm($descuento, 'PATCH')->route('admin.tienda.descuento.update',$descuento->id)->attribute('autocomplete', 'off')->open() }}
        <div class="mb-3">
          <label class="form-label" for="codigo">Codigo:</label>
          <div class="input-group">
            <input type="text" name="codigo" id="codigo" class="form-control" placeholder="Ingrese el codigo del descuento" aria-label="Codigo descuento" aria-describedby="button-addon2" value="{{ $descuento->codigo }}">
        ...
        <div class="mb-3">
          ...
          <div class="input-group mb-3">
            <div class="input-group-text">
              ...
              <input class="form-check-input mt-0" type="checkbox" name="tipo_cant" id="tipo_cant" value="0" aria-label="Checkbox for following text input" hidden {{ $descuento->tipo_cant == 'precio' ? 'checked' : '' }}>
            </div>
            <input type="number" class="form-control" name="cantidad" id="cantidad" min="0.00" step="0.01" aria-label="Text input with checkbox" value="{{ $descuento->cantidad }}">
          </div>
        </div>
        <div class="mb-3">
          ...
          <select class="form-select" id="tipo" name="tipo" aria-label="Tipo descuento">
            ...
            <option value="producto" {{ $descuento->tipo == 'producto' ? 'selected' : '' }}>Producto</option>
            <option value="general" {{ $descuento->tipo == 'general' ? 'selected' : '' }}>General</option>
            <option value="extras" {{ $descuento->tipo == 'extras' ? 'selected' : '' }}>Extras</option>
          </select>
        </div>
        <div class="mb-3">
          <label class="form-label" for="fechas">Fecha de Inicio y Fin:</label>
          <div id="fechaInicioFin" data-coreui-start-date="{{ \Carbon\Carbon::parse($descuento->fechaInicio)->format('m/d/Y') }}"
          data-coreui-end-date="{{ \Carbon\Carbon::parse($descuento->fechaFin)->format('m/d/Y') }}"></div>
        </div>
        <div class="mb-3" id="div-prod" hidden>
          ...
          <select class="form-multi-select" name="productos[]" id="productos" multiple data-coreui-search="true" data-coreui-selection-type="tags">
            @php
                $productos2 = explode(',', $descuento->productos);
              @endphp
            @foreach($productos as $id => $nombre)
            <option value="{{ $id }}" @if (in_array($id, $productos2))selected @endif>{{ $nombre }}</option>
            @endforeach
          </select>
        </div>
